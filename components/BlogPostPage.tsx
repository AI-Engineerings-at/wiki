...
/**
 * Blog-Post für beide Sprachen: /blog/<slug>/ (DE) und /en/blog/<slug>/ (EN).
 * Die Sprache kommt aus dem Frontmatter `lang`, das Dokument-Element aus dem
 * jeweiligen Wurzel-Layout — vorher trugen 8 englische Posts lang="de".
 */
export async function BlogPostPage({ slug, lang }: { slug: string; lang: 'de' | 'en' }) {
  const en = lang === 'en'
  const base = en ? '/en/blog' : '/blog'
  const post = getBlogPostBySlug(slug)
  const html = await renderMarkdown(post.content, lang)
  const readingTime = estimateReadingTime(post.content)
  const relatedPosts = getRelatedPosts(post.slug, post.tags, lang)
  const heroImage = blogImages[post.slug]

  const shareUrl = `https://wiki.ai-engineering.at${base}/${post.slug}/`
  ...
  return (
    <div className="max-w-4xl mx-auto">
      ...
      <nav className="flex items-center gap-2 text-sm text-slate-500 mb-8">
        <Link href={base} className="hover:text-blue-400 transition-colors">
          Blog
        </Link>
      ...
      <div className="border-t border-slate-800 mt-12 pt-8">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <span className="text-sm font-medium text-slate-400">{en ? 'Share this article' : 'Artikel teilen'}</span>
      ...
      <div className="mt-12 pt-8 border-t border-white/10 text-center text-sm text-slate-600">
        {en ? (
          <p>Further reading:{' '}
            ...
            {' · '}
            <a href="/en/learning-path" className="text-blue-400 hover:text-blue-300">Learning Path</a>
          </p>
        ) : (
      ...
      {relatedPosts.length > 0 && (
        <div className="mt-16">
          <h2 className="text-xl font-bold text-white mb-6">{en ? 'Related posts' : 'Verwandte Artikel'}</h2>
      ...
      <div className="mt-12 pt-8 border-t border-slate-800 text-center">
        ...
        >
          &larr; {en ? 'All posts' : 'Alle Artikel anzeigen'}
        </Link>
      </div>
    </div>
  )
