...
/**
 * Blog-Übersicht für beide Sprachen: /blog/ (DE) und /en/blog/ (EN).
 * Bis 2026-08-21 lagen 8 englische Posts unter /blog/ mit <html lang="de">
 * (Joes Augen, Fund 14); jetzt filtert das Frontmatter `lang`.
 */
export function BlogIndexPage({ lang }: { lang: 'de' | 'en' }) {
  const en = lang === 'en'
    ...
    <div className="space-y-12">
      ...
      <div>
        ...
        <p className="text-slate-400 max-w-2xl">
          {en
            ? 'Hands-on articles — local AI stack, GDPR compliance, agent orchestration and automation. No theory, only what works.'
        ...
        <div className="flex gap-4 mt-4 text-sm text-slate-500">
          <span>{posts.length} {en ? 'articles' : 'Artikel'}</span>
          <Link href={en ? '/blog' : '/en/blog'} className="hover:text-blue-400">{en ? 'German blog' : 'English blog'} &rarr;</Link>
        </div>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        <span className="text-xs text-slate-500 py-1">{en ? 'Topics:' : 'Themen:'}</span>
