'use client'

import dynamic from "next/dynamic"
import { usePathname } from "next/navigation"
import { isEnglishPath } from "../lib/alternates"

/** Lade-Hinweis in der Sprache der Seite (vorher deutsch auf 10 EN-Seiten, Joes Augen Fund 13). */
export function DiagramLoading() {
  const isEn = isEnglishPath(usePathname() || '/')
  return (
    <figure className="my-8">
      <div className="bg-white/[0.03] border border-white/[0.06] rounded-xl p-8 flex items-center justify-center min-h-[200px]">
        <div className="text-white/30 text-sm" data-diagram-loading="1">{isEn ? 'Loading diagram...' : 'Diagramm wird geladen...'}</div>
      </div>
    </figure>
  )
}

const PlantUMLDiagram = dynamic(() => import("./PlantUMLDiagram"), {
  ssr: false,
  loading: () => <DiagramLoading />,
})

export default PlantUMLDiagram
